...
type HOUR_INDEXEX = 'morning' | 'afternoon' | 'night' 
...
export class FilterUnitsService {
  ...
  filter(results: Location[], showClosed: boolean, hour: string){
    ...
    const OPEN_HOUR = OPENINGS_HOURS[hour as HOUR_INDEXEX].first
    const CLOSE_HOUR = OPENINGS_HOURS[hour as HOUR_INDEXEX].last
